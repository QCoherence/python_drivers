...
def get_data(queue_data, parameters):
    ...
    # We set the clock
    set_clock(board, parameters)

    # We set the two inputs (chanel A and B)
    set_input_control(board)

    # We set the trigger
    set_trigger(board, parameters)

    # We prepare the acquisition
    buffers = prepare_acquisition(board, parameters)

    # We launch the data acquisition
    data_acquisition(board, queue_data, parameters, buffers)
